...
class RSA:
    ...
    def __init__(self, *args, **kwargs) -> None:
        self.key_length = int(kwargs.get("key_length")[0])
    ...
    def check_keys(self) -> bool:
        ...
        if b"test" != self.decode(self.encode(b'test')):
            raise KeyError("Keys is wrong!")
        return True

    def encode(self, message: bytes) -> bytes:
        encoded = b""
        for i in range(0, len(message), __BLOCK_LENGTH__):
            block = int.from_bytes(message[i:i + __BLOCK_LENGTH__], "big")
            value = pow(block, self._data.get("pubkey")[0], self._data.get("pubkey")[1])
            encoded += value.to_bytes(__BLOCK_LENGTH__ * 2, "big")
        return encoded

    def decode(self, crypto_message: bytes) -> bytes:
        decoded = b""
        for i in range(0, len(crypto_message), __BLOCK_LENGTH__ * 2):
            block = int.from_bytes(crypto_message[i:i + __BLOCK_LENGTH__ * 2], "big")
            value = pow(block, self._data.get("privkey")[0], self._data.get("privkey")[1])
            decoded += value.to_bytes(__BLOCK_LENGTH__ * 2, "big").replace(b"\x00", b'')
        return decoded
